...
//                                                  |  $$$$$$/
	...
	function loadAssets(){
		...
		YIPLogo.src = "images/YIP_Logo_sm.png";
	}//End of loadAssets
	...
	function updateScreen(){
		...
		if(GAME_STATE === "endscreen"){
			...
			if(endScreenSlideNum === 3){
				...
				c.drawImage(HDGHLogo,710,255);
				c.drawImage(YIPLogo,950,260);
				c.drawImage(shareBtn, 726, 367);
				drawScaledImage(twitterLink,twtBtnScale,957,400);
				drawScaledImage(fbLink,fbBtnScale,1065,400);

				drawScaledImage(printTipsBtn,printBtnScale,919,525);
				drawScaledImage(retakeQuizBtn,quizBtnScale,919,630);
			}
		...
		switch(GAME_STATE){
				...
				if(modelY > dotButtonY && modelY < dotButtonY + dotButtonSize){
					for(ii=0;ii<5;ii++){
						if(modelX > dotButtonX[ii] && modelX < dotButtonX[ii] + dotButtonSize){
							plantValues[currentQuestion - 1] = ii + 1;
							dotSelected = true;
							showNext = true;
							activeDot = ii;
						}
			...
			case "endscreen":
				...
				if(endScreenSlideNum === 3){
					...
					//----------     PARTNER BUTTON     ----------
					if(modelX > 1060 && modelX < 1216){
						if(modelY > 205 && modelY < 275){
...
//    
	...
	function calculateEndPlant(){
		var plantTrack = 0;
		for(ii=0;ii<15;ii++){
			plantTrack += plantValues[ii]
		}

		//console.log("Final Score: " + plantTrack);
		
		if(plantTrack < 27){
			finalPlant = 4;
		}
		if(plantTrack > 26 && plantTrack < 39){
			finalPlant = 2;
		}
		if(plantTrack > 38 && plantTrack < 51){
			finalPlant = 3;
		}
		if(plantTrack > 50 && plantTrack < 63){
			finalPlant = 1;
		}
		if(plantTrack > 62){
			finalPlant = 0;
